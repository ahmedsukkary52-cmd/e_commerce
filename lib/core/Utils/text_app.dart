import 'package:e_commerce/core/Utils/color_app.dart';
import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:google_fonts/google_fonts.dart';

class TextApp {
  static TextStyle semiBold24Blue = GoogleFonts.poppins(
    color: ColorApp.primaryBlue,
    fontSize: 24.sp,
    fontWeight: FontWeight.w600,
  );
  static TextStyle semiBold24White = GoogleFonts.poppins(
    color: ColorApp.primaryWhite,
    fontSize: 24.sp,
    fontWeight: FontWeight.w600,
  );
  static TextStyle light16Blue = GoogleFonts.poppins(
    color: ColorApp.primaryBlue,
    fontSize: 16.sp,
    fontWeight: FontWeight.w300,
  );
  static TextStyle light16white = GoogleFonts.poppins(
    color: ColorApp.primaryWhite,
    fontSize: 16.sp,
    fontWeight: FontWeight.w300,
  );
  static TextStyle light14Blue = GoogleFonts.poppins(
    color: ColorApp.primaryBlue,
    fontSize: 14.sp,
    fontWeight: FontWeight.w300,
  );
  static TextStyle medium18Blue = GoogleFonts.poppins(
    color: ColorApp.primaryBlue,
    fontSize: 18.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle medium18White = GoogleFonts.poppins(
    color: ColorApp.primaryWhite,
    fontSize: 18.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle medium14White = GoogleFonts.poppins(
    color: ColorApp.primaryWhite,
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle medium18Gray = GoogleFonts.poppins(
    color: ColorApp.descTextBlue,
    fontSize: 18.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle medium18Red = GoogleFonts.poppins(
    color: ColorApp.redColor,
    fontSize: 18.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle medium15Red = GoogleFonts.poppins(
    color: ColorApp.redColor,
    fontSize: 15.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle light18Blue = GoogleFonts.poppins(
    color: ColorApp.primaryWhite,
    fontSize: 18.sp,
    fontWeight: FontWeight.w300,
  );
  static TextStyle light18White = GoogleFonts.poppins(
    color: ColorApp.primaryWhite,
    fontSize: 18.sp,
    fontWeight: FontWeight.w300,
  );
  static TextStyle regular18Blue = GoogleFonts.poppins(
    color: ColorApp.primaryBlue,
    fontSize: 18.sp,
    fontWeight: FontWeight.normal,
  );
  static TextStyle regular18White = GoogleFonts.poppins(
    color: ColorApp.primaryWhite,
    fontSize: 18.sp,
    fontWeight: FontWeight.normal,
  );
  static TextStyle semiBold20Blue = GoogleFonts.poppins(
    color: ColorApp.primaryBlue,
    fontSize: 20.sp,
    fontWeight: FontWeight.w600,
  );
  static TextStyle semiBold20White = GoogleFonts.poppins(
    color: ColorApp.primaryWhite,
    fontSize: 20.sp,
    fontWeight: FontWeight.w600,
  );
  static TextStyle light18Gray = GoogleFonts.poppins(
    color: ColorApp.transBlack,
    fontSize: 18.sp,
    fontWeight: FontWeight.w300,
  );
  static TextStyle light14DTB = GoogleFonts.poppins(
    color: ColorApp.descTextBlue,
    fontSize: 14.sp,
    fontWeight: FontWeight.w300,
  );
  static TextStyle medium18DarkBlue = GoogleFonts.poppins(
    color: ColorApp.darkBlue,
    fontSize: 18.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle regular14DarkBlue = GoogleFonts.poppins(
    color: ColorApp.darkBlue,
    fontSize: 14.sp,
    fontWeight: FontWeight.normal,
  );
  static TextStyle medium18DTB = GoogleFonts.poppins(
    color: ColorApp.descTextBlue,
    fontSize: 18.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle medium14DTB = GoogleFonts.poppins(
    color: ColorApp.descTextBlue,
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle medium14DarkBlue = GoogleFonts.poppins(
    color: ColorApp.darkBlue,
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle medium14Blue = GoogleFonts.poppins(
    color: ColorApp.primaryBlue,
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle regular12DarkBlue = GoogleFonts.poppins(
    color: ColorApp.darkBlue,
    fontSize: 12.sp,
    fontWeight: FontWeight.normal,
  );
  static TextStyle medium20DarkBlue = GoogleFonts.poppins(
    color: ColorApp.darkBlue,
    fontSize: 20.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle regular14DTB = GoogleFonts.poppins(
    color: ColorApp.descTextBlue,
    fontSize: 14.sp,
    fontWeight: FontWeight.normal,
  );
  static TextStyle regular14White = GoogleFonts.poppins(
    color: ColorApp.primaryWhite,
    fontSize: 14.sp,
    fontWeight: FontWeight.normal,
  );
  static TextStyle medium20White = GoogleFonts.poppins(
    color: ColorApp.primaryWhite,
    fontSize: 20.sp,
    fontWeight: FontWeight.w500,
  );
  static TextStyle oldPriceRegular11Blue = GoogleFonts.poppins(
    color: ColorApp.transparentBlue,
    fontSize: 11.sp,
    fontWeight: FontWeight.normal,
    decoration: TextDecoration.lineThrough,
    decorationColor: ColorApp.transparentBlue,
    decorationThickness: 2,
  );
}
